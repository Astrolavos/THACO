...
const loadFile = (db, day, objId) => {
  console.log(`Processing day ${day}, GEO`);
  const collection = db.collection('items');
  try {
    var result = [];
    const data = require(`./data/_${day}_geo.json`);
    result.push({
      _id: objId,
      parents: [{
        _id: objId,
      ...
      children: data.map(continent => walk(result, day, continent, [{
        _id: objId,
        group: 'GEO'
      }])),
    })
    ...
    console.log(`Error when processing ./data/_${day}_geo.json`);
    console.log(e);
  }
}
